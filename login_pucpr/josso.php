...
function jossoRequestLoginForUrl($currentUrl, $optional = FALSE) {
    ...
    $josso_agent = $jossoagent->getNewInstance();
	echo '11';
    $securityCheckUrl = $josso_agent->getBaseCode().'/josso-security-check.php';
    echo '===>'.$securityCheckUrl;
	exit;
...
function jossoSecurityCheckUrl() {
    ...
    // Get JOSSO Agent instance
    $josso_agent = & jossoagent::getNewInstance();
    $securityCheckUrl = $josso_agent->getBaseCode().'/josso-security-check-3.php';
...
function createBaseUrl() {
	...
	$link = $protocol.'://'.$host.(isset($port) ? ':'.$port : ''); 
    return $link;
